...
import EventIcon from '@mui/icons-material/EventNote'; // Import the Events icon
...
const Header = () => {
  ...
  const [notifications] = useState([
    ...
    "Update on your scheduled meeting.",
  ]);
    ...
    <div className="relative w-full z-50 bg-gradient-to-r from-blue-700 to-blue-900 text-white shadow-lg" style={{ marginTop: '0' }}>
      <div className="flex items-center justify-between p-3">
        ...
        <div className="flex items-center mr-4">
          ...
          {/* Notification Button */}
          <button
            className="ml-4 bg-blue-600 text-white p-2 rounded-lg hover:bg-blue-500 transition duration-300 focus:outline-none"
            onClick={toggleModal}
          >
      ...
      <Modal open={isModalOpen} onClose={toggleModal}>
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-80 bg-white rounded-lg shadow-lg p-4">
          <h2 className="text-lg font-semibold mb-4 text-center">Notifications</h2>
          <ul>
            {notifications.length > 0 ? (
              notifications.map((notification, index) => (
                <li key={index} className="p-2 border-b border-gray-300 last:border-b-0">
                  {notification}
                </li>
              ))
            ) : (
