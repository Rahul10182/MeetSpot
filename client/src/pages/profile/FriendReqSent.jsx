...
const FriendRequestSent = () => {
  ...
  const handleReject = async (user2ID) => {
    try {
      await axios.post('http://localhost:3000/friend/reject', { firebaseID1: user2ID , firebaseID2: firebaseID });
      fetchFriends();
    } catch (error) {
        ...
        {friends.length > 0 ? (
          friends.map((friend, index) => (
            <Card key={index} className="flex items-center mb-4 p-2">
              ...
              <IconButton color="error" aria-label="Reject" onClick={() => handleReject(friend.firebaseID)}>
                <Delete />
              </IconButton>
            </Card>
          ))
